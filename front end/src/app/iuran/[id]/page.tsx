...
async function getDataPayment($id: number) {
  const res = await fetch(BASE_URL_API + "pembayaran/" + $id);
  const data = await res.json();

  return data;
}
const EditIuran = ({ params }: { params: { id: number } }) => {
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await fetch(BASE_URL_API + "pembayaran/" + params.id, {
        method: "PUT",
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getDataPayment(params.id);
        setData(result);
        form.reset({
          penghuni_id: result?.penghuni_id || "",
          nominal: result?.jumlah_iuran || "",
          status_bayar: result?.status_bayar || "",
          jenis_iuran: result?.jenis_iuran || "",
          status_uang: result?.status_uang || "",
        });
      } catch (error) {
        console.error("Error fetching data:", error);
      }
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Edit Iuran</h1>
      <Form {...form}>
        <form
